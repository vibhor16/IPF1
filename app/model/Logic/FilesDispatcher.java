...
public class FilesDispatcher implements Serializable
{
	...
	public void dispatch_returnResults(String directoryPath)
	{
 		...
 		// System.out.println("FDOB: IP -- "+requestIP);
 		// System.out.println("FDOB: PLS: "+ProgressBar.PLS_DIRECTORIES);
 		// System.out.println("FDOB: FMB: "+ProgressBar.FMB_DIRECTORIES);
 		// System.out.println("FDOB: OTHER: "+ProgressBar.OTHER_DIRECTORIES);
...
//				PlsHandler ob=new PlsHandler();
				clientObject.getPlsHandlerObject().init();
//				ob.init(req_obj);
		 		file_package_name=PlsHandler.global_file_package_name;
// 				ob.returnResults(directoryPath,TO_SEARCH).forEach(FINAL_RETURN_RESULT::putIfAbsent);
				clientObject.getPlsHandlerObject().returnResults(directoryPath,TO_SEARCH).forEach(FINAL_RETURN_RESULT::putIfAbsent);
			}
 			else
			if(clientObject.OTHER_DIRECTORIES.contains(directoryPath) || clientObject.FMB_DIRECTORIES.contains(directoryPath))
			{
			 	System.out.println("\t[INFO]\t<"+ ++dirCount +") SEARCHING -> "+directoryPath+">");
// 		 		OtherFormatsHandler ob=new OtherFormatsHandler();
				clientObject.getOtherFormatsHanlerObject().init();
//				ob.init(req_obj);
//				LinkedHashMap<String, ArrayList<String>> temp=ob.returnResults(directoryPath,TO_SEARCH);
